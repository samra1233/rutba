...
class Database {
  ...
  public createCategory(cat: Omit<CategoryDef, 'id' | 'num'> & { id?: string; num?: string }): CategoryDef {
    const categories = this.getCategories();
    const newId = cat.id || `cat-${Date.now()}`;
    const nextNum = cat.num || (categories.length + 1).toString().padStart(2, '0');
    ...
    this.save();
    this.writeFirestoreDoc('categories', newCat.id, newCat);
    return newCat;
  }

  public updateCategory(id: string, updated: Partial<CategoryDef>): CategoryDef | undefined {
    const categories = this.getCategories();
    const cat = categories.find(c => c.id === id);
    if (cat) {
      Object.assign(cat, updated);
      this.save();
      this.writeFirestoreDoc('categories', id, cat);
      return cat;
    }
    return undefined;
  }
  ...
  public deleteCategory(id: string): boolean {
    ...
    if (index !== -1) {
      ...
      this.state.categories = categories.map((c, i) => ({ ...c, num: (i + 1).toString().padStart(2, '0') }));
      this.save();
