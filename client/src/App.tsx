...
import { ContactPage, OrderTrackingPage, PoliciesPage } from './pages/Content/ContentPages';
import { motion, AnimatePresence } from 'motion/react';
import WishlistDrawer from './components/wishlist/WishlistDrawer';
import { X, Bell, ExternalLink, ShieldCheck, Mail, Heart, ArrowLeft, Instagram, Facebook } from 'lucide-react';
import footerEmblemGold from './assets_footer_emblem.png';
...
function MainLayout() {
  ...
  return (
    <div className="min-h-screen flex flex-col justify-between select-none bg-[#080D0A] text-white">


      {/* Default browser cursor active */}
          ...
          {liveAlerts.map((alert) => (
            ...
            >
              ...
              <div className="flex-1 pr-4">
                <span className="block font-mono text-[9px] uppercase tracking-widest opacity-70 mb-0.5">RUTBA Handloom Feed</span>
      ...
      {/* 5. BRAND FOOTER */}
      {activePage === 'home' && (
        <footer className="hidden md:block relative w-full border-t border-[#C5A059]/30 shadow-2xl z-20 bg-gradient-to-b from-[#051c0e] via-[#002f15] to-[#011409] text-white">
          <div
            className="pt-8 pb-4 px-6 md:px-10 relative overflow-hidden bg-transparent"
          >
            {/* Top gold shimmer divider line */}
            <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-[#C5A059]/60 to-transparent z-10" />

            {/* Brand Logo centered */}
            <div className="relative z-10 flex justify-center mb-3">
              <img
                src={footerEmblemGold}
                alt="ROTBA Royal Gold Monogram Emblem"
                className="h-20 md:h-28 object-contain select-none pointer-events-none transition-transform duration-300 hover:scale-105 drop-shadow-[0_4px_15px_rgba(197,160,89,0.3)]"
              />
            </div>
            ...
            <div className="relative z-10 max-w-2xl mx-auto grid grid-cols-1 sm:grid-cols-2 gap-12 text-center sm:text-left justify-items-center">
              ...
              <div className="space-y-2 w-full max-w-[240px] text-center sm:text-left">
                <h3
                  className="font-serif text-[18px] font-bold text-[#E8C888] uppercase tracking-[0.2em] border-b border-[#C5A059]/30 pb-1"
                  ...
                  {[
                    ...
                    <li key={idx}>
                      <button
                        ...
                        className="w-full text-center sm:text-left py-0.5 text-stone-300 hover:text-[#C5A059] transition-colors duration-300 uppercase text-[11px] tracking-[0.12em] cursor-pointer font-bold font-mono"
                      >
                        {link.label}
                      </button>
                    </li>
                      ...
                      <button
                        onClick={() => {
                          if (link.label === 'Contact Us') {
                            addToast("You can contact us at care@rutba.com or WhatsApp: +92 300 123 4567", "info");
                          } else {
            ...
            <div className="relative z-10 flex justify-center items-center gap-6 pt-3 border-t border-[#C5A059]/25 mt-4 max-w-2xl mx-auto">
              <a
                href="https://facebook.com"
                target="_blank"
                rel="noopener noreferrer"
                className="w-8 h-8 rounded-full border border-[#C5A059]/40 bg-[#051c0e] flex items-center justify-center text-[#E8C888] hover:text-black hover:bg-[#C5A059] hover:border-[#C5A059] transition-all duration-300 hover:scale-110 shadow-sm"
                aria-label="Facebook"
              >
            ...
            <div className="relative z-10 pt-3 border-t border-[#C5A059]/20 mt-4 w-full max-w-7xl mx-auto flex flex-row flex-wrap justify-between items-center gap-4">
              {/* Copyright notice */}
              <div className="text-left text-[11px] font-mono tracking-widest uppercase text-stone-400 flex items-center gap-4 flex-wrap">
                <span>&copy; {new Date().getFullYear()} RUTBA LUXURY UNSTITCHED. ALL RIGHTS RESERVED.</span>
              </div>

              {/* Payment Methods Row */}
              <div className="flex items-center gap-2.5 justify-end ml-auto flex-wrap">

                {/* Visa Icon */}
                <div className="w-12 h-7.5 rounded overflow-hidden flex items-center justify-center p-0.5 shadow-sm hover:scale-105 transition-transform duration-300 bg-white border border-neutral-200">
                ...
                {/* Mastercard Icon */}
                <div className="w-12 h-7.5 rounded bg-white border border-neutral-200 flex items-center justify-center p-1.5 shadow-sm hover:scale-105 transition-transform duration-300">
                ...
                {/* Discover Icon */}
                <div className="w-12 h-7.5 rounded bg-white border border-neutral-200 flex items-center justify-center p-1 shadow-sm hover:scale-105 transition-transform duration-300">
